...
// This component is a direct adaptation of DS_ExamPage.jsx
// It works for any subject that uses a simple, test-case-based validation loop
// where the backend expects a subject name and a list of test cases in the JSON.
...
const DeepLearning_ExamPage = () => {
  ...
  // --- Start of Callback and Effect Hooks ---
  ...
  const handleAdminValidationSubmit = useCallback(async () => {
    ...
    if (examParts.length > 0) {
      const partsStatus = examParts.map((part) => {
        ...
        const partInfo = {
          ...
          status: partStatus === true ? "PASSED" : partStatus === false ? "FAILED" : "NOT_ATTEMPTED",
        };
        
        if (partStatus === true) {
          partInfo.reason = `Part ${part.part_id || part.id} passed validation tests.`;
        } else if (partStatus === false) {
          partInfo.reason = `Part ${part.part_id || part.id} failed validation tests.`;
        } else {
          partInfo.reason = `Part ${part.part_id || part.id} was not validated.`;
        }
      ...
      passed = allPartsValidated || (examParts.length === 1 && anyPartValidated);
      debugInfo.overallPassed = passed;

      if (allPartsValidated) {
        debugInfo.reasons.push(`✅ All ${examParts.length} part(s) passed validation.`);
      } else if (anyPartValidated && examParts.length === 1) {
        debugInfo.reasons.push(`✅ Single part question passed validation.`);
      } else {
        const failedParts = partsStatus.filter((p) => !p.passed && p.status !== "NOT_ATTEMPTED");
        const notAttemptedParts = partsStatus.filter((p) => p.status === "NOT_ATTEMPTED");
        
        if (failedParts.length > 0) {
          debugInfo.reasons.push(`❌ ${failedParts.length} part(s) failed: ${failedParts.map(p => p.partTitle).join(", ")}`);
        }
        if (notAttemptedParts.length > 0) {
          debugInfo.reasons.push(`⚠️ ${notAttemptedParts.length} part(s) not attempted: ${notAttemptedParts.map(p => p.partTitle).join(", ")}`);
        }
      ...
      const statusInfo = {
        questionId: validationQuestionId,
        passed: status === true,
        status: status === true ? "PASSED" : status === false ? "FAILED" : "NOT_ATTEMPTED",
      };
      if (status === true) {
    ...
    try {
      ...
      if (pendingValidation) {
        ...
        validationResultData.questionId = pendingId || normalizedQuestionId;
      }
      ...
      if (!passed) {
        ...
        try {
          const deleteResponse = await fetch(
            `${API_BASE_URL}/api/admin/delete-question/${subject}/${level}/${encodeURIComponent(validationResultData.questionId)}`,
            { method: "DELETE" }
          );
          const deleteData = await deleteResponse.json();
          if (!deleteResponse.ok) {
            console.error("❌ Failed to delete question:", deleteData.message);
            debugInfo.reasons.push(`⚠️ Warning: Failed to delete question: ${deleteData.message}`);
          } else {
          ...
          debugInfo.reasons.push(`⚠️ Error deleting question: ${deleteError.message}`);
        }
      } else {
        ...
        try {
          const response = await fetch(
            `${API_BASE_URL}/api/admin/update-validation-status`,
            {
              ...
              body: JSON.stringify({
                subject,
                level,
                questionId: validationResultData.questionId,
                isValidated: true,
              }),
            }
          );
          ...
          if (!response.ok) {
            console.error("❌ Failed to update validation status:", data.message);
            debugInfo.reasons.push(`⚠️ Warning: Failed to update validation status: ${data.message}`);
          } else {
          ...
          debugInfo.reasons.push(`⚠️ Error updating validation status: ${updateError.message}`);
        }
      }

      localStorage.setItem("questionValidationResult", JSON.stringify(validationResultData));
      const modalResult = {
        passed,
        subject,
        level,
    ...
    const fetchAndPrepareQuestions = async () => {
      try {
        const res = await fetch(
          `${API_BASE_URL}/api/questions/${subject}/${level}`
        );
        if (!res.ok)
        ...
        if (isAdminValidationMode) {
          ...
          const questionToValidate = data.find((q) => {
            ...
            const matchesSimpleId = qId === normalizedValidationId || String(qId) === normalizedValidationId || Number(qId) === Number(normalizedValidationId);
            const matchesPartId = q.parts && q.parts.some((p) => {
              const partIdStr = `${q.id}_${p.part_id}`;
              return partIdStr === normalizedValidationId || partIdStr === String(normalizedValidationId);
            });
            return matchesSimpleId || matchesPartId;
          });

          if (questionToValidate) {
            if (questionToValidate.parts) {
              const partToValidate = questionToValidate.parts.find(
                (p) => `${questionToValidate.id}_${p.part_id}` === normalizedValidationId
              );
              data = partToValidate ? [{ ...questionToValidate, ...partToValidate, id: `${questionToValidate.id}_${partToValidate.part_id}` }] : [questionToValidate];
            } else {
            ...
            throw new Error(`Question with ID "${normalizedValidationId}" not found in this level.`);
          }
        }
  ...
  }, [subject, level, sessionId, isAdminValidationMode, validationQuestionId]);
    ...
    <div className="flex flex-col h-screen bg-gray-100">
      ...
      <>
        <header className="flex sticky top-0 z-30 items-center min-h-[70px] px-4 bg-white border-b border-gray-200">
          ...
          >
            {isAdminValidationMode ? "Validate" : "Finish Now"}
          </button>
        </header>
            ...
            <div className="grid grid-cols-2 gap-3">
              {examParts.map((part, index) => (
                <button
                  ...
                  title={`Question ${index + 1}: ${part.title || "Untitled"}`}
                >
                  {index + 1}
                </button>
              ))}
          ...
          <main className="flex-grow flex flex-col min-h-0 bg-gray-50 overflow-auto">
            {currentPart ? (
              ...
              <div className="flex flex-grow items-center justify-center">
                { examParts.length === 0 && hasExamStarted ? <p>No questions found for this level.</p> : <Spinner /> }
              </div>
            )}
          </main>
        </div>
...
export default DeepLearning_ExamPage;