...
// This is an exact copy of the AlertCard from DS_ExamPage
...
// This is an exact copy of the CodeCell from DS_ExamPage
const CodeCell = ({ question, mainTask, cellCode, onCodeChange, onRun, onValidate, cellResult, isExecuting, isValidated, customInput, onCustomInputChange, isCustomInputEnabled, onToggleCustomInput, isSessionReady, securityConfig }) => {
    ...
    const renderOutputContent = () => {
        ...
        if (isValidated === false) {
            return (<pre className="bg-yellow-50 text-yellow-700 rounded p-3 font-mono whitespace-pre-wrap break-words border border-yellow-200">Your code ran without errors, but the output did not match the expected result for one or more test cases.</pre>);
        }
        ...
        <div className="flex flex-col md:flex-row h-full w-full p-4 overflow-hidden">
                ...
                {visibleTestCase && (
                    <div className="mt-8 font-medium">
                        ...
                        <div className="bg-gray-50 border border-gray-200 rounded-md p-4 text-sm">
                            <p className="font-semibold">Input:</p><pre className="bg-white p-2 rounded mt-1 font-mono text-slate-600">{visibleTestCase.input}</pre>
                            <p className="font-semibold mt-3">Expected Output:</p><pre className="bg-white p-2 rounded mt-1 font-mono text-slate-600">{visibleTestCase.output}</pre>
                        </div>
                    </div>
                )}
                <div className="mt-8 font-medium">
            ...
            <div className="flex-1 flex flex-col bg-white border border-slate-200 rounded-lg overflow-hidden">
                <div className="flex-grow min-h-[200px] max-h-[70%] relative">
                    ...
                    <Editor height="100%" language="python" theme="vs-dark" value={cellCode} onChange={(value) => onCodeChange(value || "")} options={{ minimap: { enabled: false }, fontSize: 14, scrollBeyondLastLine: false, wordWrap: "on", padding: { top: 15 }, }} onMount={(editor, monaco) => { if (securityConfig?.paste) { editor.addCommand(monaco.KeyMod.CtrlCmd | monaco.KeyCode.KeyV, () => { alert("Pasting is disabled during the exam."); }); } }} />
                </div>
...
// <<< START: ONLY CHANGE IS HERE >>>
const LLM_ExamPage = () => {
// <<< END: ONLY CHANGE IS HERE >>>
  ...
  const handleSubmissionModalConfirm = useCallback(async () => {
    if (document.fullscreenElement) { await document.exitFullscreen().catch((err) => console.error("Error exiting fullscreen:", err)); }
  ...
  const handleSubmitExam = useCallback(async () => {
    ...
    setIsSubmitting(true); setIsFinalSubmission(true);
    const answers = examParts.map((p) => ({ questionId: p.taskId || p.id, partId: p.part_id || p.id, code: allCode[p.id] || "", passed: !!validationStatus[p.id] }));
    try {
      const response = await fetch(`${API_BASE_URL}/api/evaluate/submit`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId, username: user.username, subject, level, answers, }), });
      if (!response.ok) throw new Error(`Server responded with status: ${response.status}`);
      const data = await response.json();
      if (data.updatedUser) { updateUserSession(data.updatedUser); }
      setPerformanceReport({ answers: answers, performance: data.performance_metrics || [] });
    } catch (error) {
      console.error("Submission error:", error);
      setSubmissionResult(`An error occurred during final submission: ${error.message}`);
    } finally {
    ...
    if (hasExamStarted) {
      const timer = setInterval(() => {
        setTimeLeft((prevTime) => {
          if (prevTime <= 1) { clearInterval(timer); handleSubmitExam(); return 0; }
          return prevTime - 1;
        });
      }, 1000);
  ...
  const showWarningPopup = useCallback((message) => { setWarningMessage(message); }, []);
  const { startExam, reEnterFullScreen } = useFullScreenExamSecurity(handleSubmitExam, 3, showWarningPopup, securityConfig, isFinalSubmission);
  ...
  const handleStartExamClick = async () => {
    if (securityConfig?.fullscreen) {
      try { await document.documentElement.requestFullscreen(); startExam(); setHasExamStarted(true); }
      catch (err) { alert("Fullscreen is required to start the exam. Please enable it in your browser and try again."); }
    } else {
      startExam(); setHasExamStarted(true);
    }
  };

  const handleWarningConfirm = () => { setWarningMessage(""); reEnterFullScreen(); };
  ...
  useEffect(() => {
    ...
    const fetchAndPrepareQuestions = async () => {
      try {
        ...
        const initialCode = {}; data.forEach((q) => { initialCode[q.id] = q.starter_code || ""; });
  ...
  const handleRunCell = async (partId) => {
    ...
    const userInput = useDefaultInput ? currentPart.test_cases?.[0]?.input || "" : customInputs[partId] || "";
    try {
      const res = await fetch(`${API_BASE_URL}/api/evaluate/run`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId, cellCode, userInput, username: user.username, subject, level, questionId: currentPart.taskId || currentPart.id, partId: currentPart.part_id || null, }), });
  ...
  const handleValidateCell = async (partId) => {
    ...
    const cellCode = allCode[partId] || "pass";
    try {
      const res = await fetch(`${API_BASE_URL}/api/evaluate/validate`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId, username: user.username, subject, level, questionId: currentPart.taskId || currentPart.id, partId: currentPart.part_id || null, cellCode, }), });
      ...
      {!hasExamStarted ? (
        ...
        <>
          <header className="flex sticky top-0 z-30 items-center min-h-[70px] px-4 bg-white border-b border-gray-200">
            <div className="ml-2 px-2 mr-2 items-center content-center rounded-md bg-gray-100 h-[55px]"><h4 className="font-bold text-lg text-slate-800">{subject.charAt(0).toUpperCase() + subject.slice(1)} Exam - Level {level}</h4></div>
            ...
            <div className="flex-grow flex justify-center items-center"><span className="text-lg font-semibold text-gray-800">Time Left: {formatTime(timeLeft)}</span></div>
          ...
          <div className="flex flex-grow min-h-0">
            <div className="w-[180px] flex-shrink-0 bg-white border-r border-gray-200 p-6 overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-3">{examParts.map((part, index) => (<button key={part.id} onClick={() => setCurrentQuestionIndex(index)} className={`flex items-center justify-center w-16 h-16 rounded-lg text-white font-bold text-xl ${getQuestionBoxColor(part.id, index)}`} title={`Question ${index + 1}: ${part.title || "Untitled"}`}>{index + 1}</button>))}</div>
              <div className="bg-white min-h-0 max-w-[200px] mt-8 p-4 rounded-2xl shadow-lg border border-gray-100">
                <div className="space-y-2">
                  <div className="flex items-center gap-2"><div className="h-4 w-4 bg-purple-600 rounded-sm flex-shrink-0 shadow-sm"></div><span className="text-xs font-semibold text-gray-800 leading-tight">Current Part</span></div>
            ...
            <main className="flex-grow flex flex-col min-h-0 bg-gray-50 overflow-auto">
              {currentPart ? (<div className="flex flex-grow w-full"><CodeCell key={currentPart.id} question={currentPart} mainTask={mainTask} cellCode={allCode[currentPart.id] || ""} onCodeChange={(value) => handleCodeChange(currentPart.id, value)} onRun={() => handleRunCell(currentPart.id)} onValidate={() => handleValidateCell(currentPart.id)} cellResult={cellResults[currentPart.id]} isExecuting={isExecuting || isSubmitting} isValidated={validationStatus[currentPart.id]} customInput={customInputs[currentPart.id] || ""} onCustomInputChange={(value) => handleCustomInputChange(currentPart.id, value)} isCustomInputEnabled={!!isCustomInputEnabled[currentPart.id]} onToggleCustomInput={() => handleToggleCustomInput(currentPart.id)} isSessionReady={isSessionReady} securityConfig={securityConfig}/></div>) : (<div className="flex flex-grow items-center justify-center"><Spinner /></div>)}
            </main>
          </div>
