...
// This component is a direct adaptation of DS_ExamPage.jsx for the R Programming subject.
// The core logic for fetching questions and validation is identical.
    ...
    if (isValidated === false) {
      return (
        <pre className="bg-yellow-50 text-yellow-700 rounded p-3 font-mono whitespace-pre-wrap break-words border border-yellow-200">
          Your code ran without errors, but the output did not match the expected result for one or more test cases.
        </pre>
      );
    }
    ...
    <div className="flex flex-col md:flex-row h-full w-full p-4 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col bg-white border border-slate-200 rounded-lg overflow-hidden">
        <div className="flex-grow min-h-[200px] max-h-[70%] relative">
          ...
          <Editor
            height="100%"
            language="r" // <<< ONLY CHANGE: Set language to "r" for proper syntax highlighting
            theme="vs-dark"
            value={cellCode}
...
// Main Component name changed to R_ExamPage
const R_ExamPage = () => {
  ...
  const handleAdminValidationSubmit = useCallback(async () => {
    ...
    try {
      ...
      localStorage.setItem("questionValidationResult", JSON.stringify(validationResult));
  ...
  }, [isSubmitting, validationStatus, validationQuestionId, subject, level, navigate, examParts]);
  ...
  const handleSubmitExam = useCallback(async () => {
    ...
    const passedCount = Object.values(validationStatus).filter(status => status === true).length;
  ...
  }, [isSubmitting, examParts, allCode, user, subject, level, sessionId, validationStatus, updateUserSession, isAdminValidationMode, handleAdminValidationSubmit]);
